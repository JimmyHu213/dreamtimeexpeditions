...
- Always `git fetch` the latest before creating a new branch or worktree, so it branches off an up-to-date `main` rather than a stale local ref.
- Rebase onto the latest base before pushing (`git fetch && git rebase origin/main`). This keeps history linear and surfaces conflicts locally instead of in the PR.
